...
png("plot1.4_library_duplicates_percentage.png",height=2000,width=1200,res=300)
plot=ggplot(ref.dedup,aes(x=class,y=ratio))+
    ...
    scale_x_discrete(name="")+
	ggtitle("Boxplot of library duplicates\npercentage from FastQC")+
	...
	theme(plot.title=element_text(size=14,family="Tahoma",face="bold",hjust=0.5),
		...
		axis.text.y=element_text(size=10,face="bold"))
if(dedup$ratio<50) {
	plot+scale_y_continuous(name="Percentage of library duplicates",limits=c(0,50))
} else {
	plot+scale_y_continuous(name="Percentage of library duplicates")
}
dev.off()

# enrichment
ref.enrich=read.table('/home/chengl/Update/visual_reference/merged_top20k_enrichment_ratio.txt',header=T) ####
...
ref.enrich2=read.table('/home/chengl/Update/visual_reference/merged_coding_promoter_peak_enrichment.txt',header=T) ####
...
ref.enrich3=read.table('/home/chengl/Update/visual_reference/merged_all_peak_enrichment.txt',header=T) ####
...
# mapping status
ref.map=read.table('/home/chengl/Update/visual_reference/merged_mapping_status.txt',header=T,sep='\t') ####
...
ref.dicho=read.table('/home/chengl/Update/visual_reference/merged_bg_dichoto.txt',header=T,sep='\t')
...
ref.chr=read.table('/home/chengl/Update/visual_reference/merged_chrom_count.txt',header=T,sep='\t')
...
ref.peak=read.table('/home/chengl/Update/visual_reference/merged_saturation_collection.txt',header=T,sep='\t')
ref.peak=ref.peak[,-1]
