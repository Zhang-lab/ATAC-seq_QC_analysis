...
png("plot1.4_deduplication_level.png",height=2000,width=3300,res=300)
plot=ggplot(ref.dedup,aes(x=deduplication_percentage,y=..scaled..,fill=`Reference: ENCODE PE`))+
	...
	theme(plot.title=element_text(size=14,family="Tahoma",face="bold",hjust=0.5),
		...
		legend.position="bottom")
if(dim(dedup)[1]==2) {
	plot+geom_vline(xintercept=dedup[2,2],linetype="dotted",size=1,color="#56B4E9")+
...
png("plot4.2_peak_enrichment_ratio.png",height=2300,width=2600,res=300)
options(warn=-1)
...
png("plot4.3_noduplication_ratio.png",height=1600,width=2300,res=300)
...
png("plot4.1_RUP.png",height=1600,width=2300,res=300)
...
colnames(ref.map)[1:2]=c("Total mapped reads","Non-redundant uniquely mapped reads")
ref.map=stack(ref.map[,1:2])
colnames(ref.map)[2]="Reference: ENCODE PE"

png("plot3.1_library_reads_distri.png",height=2000,width=3000,res=300)
plot=ggplot(ref.map,aes(x=values,y=..scaled..,fill=`Reference: ENCODE PE`))+
	...
	scale_fill_manual(values=c(`Non-redundant uniquely mapped reads`="#E69F00",`Total mapped reads`="#56B4E9"))+
	theme(plot.title=element_text(size=14,family="Tahoma",face="bold",hjust=0.5),
		text=element_text(size=12,family="Tahoma"),
		axis.title=element_text(face="bold"),
		axis.text.x=element_text(size=10,face="bold"),
...
if(map[1,1]<5e+7) {
	plot+scale_x_continuous(name="Number of reads",limits=c(0,10e+7))+
		geom_text(aes(x=map[1,2],y=1.2,label=paste("Sample non-redundant\nuniquely mapped reads",map[1,2],sep="\n")),hjust=-0.1,size=3.5,family="Tahoma",fontface="bold",color="#E69F00")+
		geom_text(aes(x=map[1,1],y=1.05,label=paste("Sample total mapped reads",map[1,1],sep="\n")),hjust=-0.1,size=3.5,family="Tahoma",fontface="bold",color="#56B4E9")
} else if(map[1,1]<9e+7){
	plot+scale_x_continuous(name="Number of reads",limits=c(0,10e+7))+
		geom_text(aes(x=map[1,2],y=1.2,label=paste("Sample non-redundant\nuniquely mapped reads",map[1,2],sep="\n")),hjust=-0.1,size=3.5,family="Tahoma",fontface="bold",color="#E69F00")+
		geom_text(aes(x=map[1,1],y=1.05,label=paste("Sample total mapped reads",map[1,1],sep="\n")),hjust=1.1,size=3.5,family="Tahoma",fontface="bold",color="#56B4E9")
} else {
...
png("plot4.6_background_noise.png",height=2000,width=3300,res=300)
options(warn=-1)
