...
ref.enrich3=read.table(paste(refpath,'merged_new_enrichment.txt',sep='/'),header=T) ####
enrich3=read.table(paste("new_enrichment",name,sep="_"),header=T)
...
map=read.table(paste("mapping_status",name,sep="_"),header=T,sep='\t')
samples=as.numeric(c(map$total,map$mapped,map[,6],map[,8]))
...
refer=c(refer,paste(round(mean(ref.chr[20,]/ref.map[,8]),4)*100,'%(SD:',round(sd(ref.chr[20,]/ref.map[,8]),4)*100,'%)',sep=''))
refer=c(refer,paste(round(mean(ref.chr[21,]/ref.map[,8]),4)*100,'%(SD:',round(sd(ref.chr[21,]/ref.map[,8]),4)*100,'%)',sep=''))
refer=c(refer,paste(round(mean(ref.chr[22,]/ref.map[,8]),4)*100,'%(SD:',round(sd(ref.chr[22,]/ref.map[,8]),4)*100,'%)',sep=''))

chr=read.table(paste("chrom_count",name,sep="_"))
samples=c(samples,paste(round(chr[20:22,3]/map[,8],4)*100,'%',sep=''))
library=data.frame(samples,refer)
colnames(library)=c("Sample","ENCODE PE")
rownames(library)=c("Total reads","Mapped reads","Useful reads","Non-redundant uniquely mapped reads","Percentage of reads in chrM","Percentage of reads in chrX","Percentage of reads in chrY")
...
samples=as.numeric(saturate[11,2])
...
library=data.frame(samples,refer)
colnames(library)=c("Sample","ENCODE PE")
rownames(library)=c("Number of peaks","Reads under peaks ratio","Enrichment ratio in coding promoter regions","Normalized enrichment ratio","Percentage of background RPKM smaller than 0.3")
