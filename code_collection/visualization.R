...
samples=c(samples,paste(round(chr[20:22,3]/map[,8],4)*100,'%',sep=''),map[,8],useful[,5])
library=data.frame(samples,refer)
colnames(library)=c("Sample","ENCODE PE")
rownames(library)=c("Total reads","Mapped reads","Non-redundant uniquely mapped reads","Percentage of reads in chrM","Percentage of reads in chrX","Percentage of reads in chrY","Useful reads","Single-end reads")
...
library=data.frame(samples,refer)
colnames(library)=c("Sample","ENCODE PE")
rownames(library)=c("Useful reads ratio","Number of peaks","Reads under peaks ratio","Enrichment ratio in coding promoter regions","Normalized enrichment ratio","Percentage of background RPKM smaller than 0.3")
