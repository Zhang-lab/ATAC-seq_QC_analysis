...
h) echo "usage:  path-to-pipe/pipe.sh  -g <hg38/hg19/mm10/mm9/danRer10> -r <PE/SE> -o <read_file1>  -p <read_file2> -i <ifr_parameter> (see below)
"
echo "If you don't want to run IFR finding step, please just ignore the -i option"
echo "Insertion free region finding will use default parameters only if -i specified as 0:
    min_lfp=5
    max_lfp=15
    step_lfp=2
    min_lsh=50
    ...
    p_cutoff=0.05
    "
echo "If you want to specify your own parameter, please make sure they are in the same order and seperated by comma
...
s2.2_distri () {
    ...
    cat <(samtools view -H output.sam) <(awk '$3=="chrM" && $5>=10' output.sam) | $methylQA density -S -r -o temp $chrom_size  -
    unique_chrM=`grep chrM temp.extended.bed | wc -l`
    unique_chrM_ratio=`echo "scale=4; $unique_chrM / $map_uniq" | bc -l`
    echo -e "unque_mapped\tchrM\tunique_chrM_ratio" > 'step2.2_unique_chrM_ratio_'$name'.result'
...
s3.1_methylQA () {
    ...
    awk '$1<=500'  'step3.1_methylQA_'*$name'.insertdistro'  | sort -n | uniq -c | awk '{print $2,$1}' > 'step3.1_insertion_distri_'$name'.result'
...
# 4.3, PBC calculation
s4.3_pbc () {
    $methylQA atac -r -o PBC_calculation  $chrom_size  'step2.1_trimed_'$name'.bam'
    ...
    rm PBC*
    echo -e "file\ttotal\tmapped\tmapped_ratio\tuniq_mapped\tnon-redundant_uniq_mapped\teffect_ratio\tuseful_reads\tPBC\tnodup_ratio\tnumber_of_reads_under_peak\trup_ratio\treplicate_dif\tbefore_dedup" >  'QC_data_collection_'$name'.result'
    echo -e "$name\t$raw_reads\t$map_mapped\t$mapped_ratio\t$map_uniq\t$map_effect\t$effect_ratio\t$useful\t$PBC\t$nodup_ratio\t$sum\t$ratio\t$dif\t$before_dedup" >>  'QC_data_collection_'$name'.result'
    mv 'QC_data_collection_'$name'.result'  ./'QC_Atac_data_collection_'$name

    if [ -z $name ] || [ -z $raw_reads ] || [ -z $map_mapped ] || [ -z $mapped_ratio ] || [ -z $map_uniq ] || [ -z $useful ] || [ -z $map_effect ] || [ -z $effect_ratio ] || [ -z $PBC ] || [ -z $nodup_ratio ] || [ -z $sum ]|| [ -z $ratio ]|| [ -z $dif ] || [ -z $before_dedup ]
    then
        echo "step4.3, sumarizing result process fail......" >> QC_pipe_processing.log
    else
        echo "step4.3, sumarizing result process done" >> QC_pipe_processing.log
    fi    
}
...
s4.6_visualization () {
    ...
    if [ $? == 0 ] 
        then
        echo "step4.7, plot process done" >> ../QC_pipe_processing.log
    else 
        echo "step4.7, plot process fail......" >> ../QC_pipe_processing.log
    fi
    ...
    mv $name'_report.txt' ../QC_$name'_report.txt'
    cd ..
...
s4.7_ifr_finding () {
    ...
    rm xa*bed list
    cat IFR*txt | sed "s/\"//g" | sort -k1,1V -k2,2n | awk '{print $1"\t"$2"\t"$3"\t""found_IFR_"NR"\t"$4"\t"".""\t"$5}' > "step4.7_found_IFR_"$name".bed"
    rm IFR*txt
}
