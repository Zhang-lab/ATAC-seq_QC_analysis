...
s4.6_visualization () {
    #summarize results
    echo -e "file\ttotal\twritten_reads\tmapped\tmapped_ratio\tuniq_mapped\tnon-redundant_uniq_mapped\teffect_ratio\tfastqc_dedup\tafter_align_dedup\tnumber_of_reads_under_peak\trup_ratio\tsub10M_enrichment\tcoding_enrichment\tbg_gt37_percentage" >  'QC_data_collection_'$name'.result'
    echo -e "$name\t$raw_reads\t$written_reads\t$map_mapped\t$mapped_ratio\t$map_uniq\t$map_effect\t$effect_ratio\t$before_dedup\t$nodup_ratio\t$sum\t$ratio\t$enrichment\t$enrichment_ratio\t$ra_more" >>  'QC_data_collection_'$name'.result'
