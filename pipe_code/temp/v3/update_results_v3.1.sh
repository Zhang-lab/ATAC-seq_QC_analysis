# update atac-seq output to version 3.1 results 
# requires: bam file, step1.1*trimlog
# run on the parent folder of Processed*
...
echo "files need to rerun from beginning" > need_to_rerun.txt
...
s4.6_visualization () {
    #summarize results
    echo -e "file\ttotal\twritten_reads\tmapped\tmapped_ratio\tuniq_mapped\tnon-redundant_uniq_mapped\teffect_ratio\tfastqc_dup\tafter_align_dup\tnumber_of_reads_under_peak\trup_ratio\tsub10M_enrichment\tcoding_enrichment\tbg_gt37_percentage" >  'QC_data_collection_'$name'.result'
    ...
    if [ -z $name ] || [ -z $raw_reads ] || [ -z $map_mapped ] || [ -z $mapped_ratio ] || [ -z $map_uniq ] || [ -z $written_reads ] || [ -z $map_effect ] || [ -z $effect_ratio ] || [ -z $PBC ] || [ -z $nodup_ratio ] || [ -z $sum ]|| [ -z $ratio ]|| [ -z $dif ] || [ -z $before_dedup ]
    then
...
date
for file in `ls -d Processed_*`
do
    name=`echo ${file#Processed_}`
    echo "processing $name start time is `date`"
    cd $file

    # 1. check trimlog and bam files
    [ ! -f step1.1_${name}_cutadapt_${types}.trimlog ] && [ ! -f step2.1_trimed_${name}.bam ] && echo "missing essential files for $name" && echo $file >> need_to_rerun.txt && cd .. && exit 1

    # 2. prepare until after-align
    if [[ $types == PE ]];
        then
        ...
        bedtools bamtofastq -i step2.1_trimed_${name}.bam -fq step1.1_trimed_${name}_1.fastq -fq2 step1.1_trimed_${name}_2.fastq
    elif [[ $types == SE ]];
        then
        ...
        written_reads=`echo ${temp2//,}`
        bedtools bamtofastq -i step2.1_trimed_${name}.bam -fq step1.1_trimed_${name}.fastq
    fi

    # 3. clera results and rerun
    find . -maxdepth 1 ! -name "step1.1*" ! -name "step2.1*" |  xargs rm -r
