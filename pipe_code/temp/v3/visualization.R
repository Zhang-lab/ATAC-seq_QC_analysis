...
ref.map$after_align_dedup=1-ref.map$after_align_dedup
map$after_align_dedup=1-map$after_align_dedup

png("plot4.3_PCR_duplicates_percentage.png",height=2000,width=1400,res=300)
options(warn=-1)
plot=ggplot(ref.map,aes(x=class,y=100*after_align_dedup,fill=class))+
  stat_boxplot(geom="errorbar",size=1,width=0.3,aes(colour=class))+
  geom_boxplot(outlier.shape=NA,width=0.3,lwd=1,fatten=1,aes(colour=class))+
  scale_x_discrete(name="")+
  ...
  theme(plot.title=element_text(size=14,family="Tahoma",face="bold",hjust=0.5),
        ...
        axis.text.x=element_text(size=10,face="bold",colour=c(`ENCODE PE`="black",Sample="red")),
        axis.text.y=element_text(size=10,face="bold"))
if(1-map$after_align_dedup*100<30) {
...
if(genome!="danRer10") {
  ...
  refer=c(refer,paste(round(mean(ref.chr[21,]/ref.map[,7]),4)*100,'%(SD:',round(sd(ref.chr[21,]/ref.map[,7]),4)*100,'%)',sep=''))
  refer=c(refer,paste(round(mean(ref.chr[22,]/ref.map[,7]),4)*100,'%(SD:',round(sd(ref.chr[22,]/ref.map[,7]),4)*100,'%)',sep=''))
...
refer=c(refer,paste(round(mean(1-ref.map$after_align_dedup),4)*100,'%(SD:',round(sd(1-ref.map$after_align_dedup),4)*100,'%)',sep=''))
samples=c(paste(round(ref.dedup[which(ref.dedup$class=='Sample'),2],2),'%',sep=''),paste((1-map$after_align_dedup)*100,'%',sep=''))
library=data.frame(samples,refer)
colnames(library)=c("Sample","ENCODE PE")
...
part9=data.frame(round(ref.dedup[which(ref.dedup$class=='Sample'),2],2)/100,(1-map$after_align_dedup))
